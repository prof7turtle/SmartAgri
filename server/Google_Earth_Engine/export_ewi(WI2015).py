...
GCS_FILE_NAME = 'Farm_WI2015_TimeSeries_with_LatLon'
...
# 2. Function to calculate WI2015
def getWI2015(image):
    wi2015 = image.normalizedDifference(['B3', 'B5']).rename('wi2015')
    return wi2015.copyProperties(image, ['system:time_start'])

# 3. Map WI2015 function
wi2015_series = s2.map(getWI2015)
...
def create_feature(img):
    ...
    wi2015_value = ee.Number(mean_dict.get('wi2015'))
...
try:
    ...
    df.dropna(subset=['wi2015'], inplace=True)
    ...
    ax.plot(df['date'], df['wi2015'], marker='o', linestyle='-', color='blue', label='Mean WI2015')
    ax.set_title('WI2015 Time Series for Farm AOI (Jan - Apr 2025)', fontsize=16)
    ax.set_xlabel('Date')
    ax.set_ylabel('WI2015')
    ax.set_ylim(-1, 1)
    ax.legend()
    ax.grid(True)
    fig.autofmt_xdate()
