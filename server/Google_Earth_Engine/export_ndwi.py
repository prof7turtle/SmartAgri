...
GCS_FILE_NAME = 'Farm_NDWI_TimeSeries'
...
# 2. Define NDWI calculation function
def getNDWI(image):
...
# 3. Map NDWI function
ndwi_series = s2.map(getNDWI)

# 4. Convert to FeatureCollection with date + mean NDWI
def create_ndwi_feature(img):
    ...
    ndwi_value = ee.Number(mean_dict.get('NDWI'))
    return ee.Feature(None, {
        'date': img.date().format('YYYY-MM-dd'),
        'ndwi': ndwi_value
    })

feature_collection = ee.FeatureCollection(ndwi_series.map(create_ndwi_feature))

# 5. Export to GCS
print(f"\nStarting export to GCS bucket: '{GCS_BUCKET_NAME}'...")
task_gcs = ee.batch.Export.table.toCloudStorage(
    collection=feature_collection,
    description='Export_NDWI_to_GCS',
...
# 6. Plot NDWI Time Series
try:
    df = pd.read_csv(file_path)
    df['date'] = pd.to_datetime(df['date'])
    df.dropna(subset=['ndwi'], inplace=True)
    df.sort_values('date', inplace=True)

    plt.style.use('seaborn-v0_8-whitegrid')
    fig, ax = plt.subplots(figsize=(14, 7))

    ax.plot(df['date'], df['ndwi'], marker='o', linestyle='-', color='green', label='Mean NDWI')
    ax.set_title('Mean NDWI Time Series (Oct 2024 - Apr 2025)', fontsize=16)
    ax.set_xlabel('Date', fontsize=12)
    ax.set_ylabel('NDWI', fontsize=12)
    ax.set_ylim(-1, 1)
    ax.legend()
    ax.grid(True)
