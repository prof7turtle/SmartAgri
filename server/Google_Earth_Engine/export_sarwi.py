...
GCS_FILE_NAME = 'Farm_SARWI_TimeSeries_with_LatLon'
...
def create_feature(img):
    ...
    sarwi_value = ee.Number(mean_dict.get('sarwi'))
...
try:
    ...
    df.dropna(subset=['sarwi'], inplace=True)
    ...
    ax.plot(df['date'], df['sarwi'], marker='o', linestyle='-', color='blue', label='Mean SARWI')
    ax.set_title('SARWI Time Series for Farm AOI (Jan - Apr 2025)', fontsize=16)
    ax.set_xlabel('Date')
    ax.set_ylabel('SARWI')
    ax.set_ylim(-1, 1)
    ax.legend()
